...
- `school-planner login <slug>` opens a browser so the parent logs a kid into ClassLink once. Never ask the parent for a password in chat; they type it into the browser, or into `.env` as `CLASSLINK_USER_<SLUG>` / `CLASSLINK_PASS_<SLUG>` for `login --auto`.
